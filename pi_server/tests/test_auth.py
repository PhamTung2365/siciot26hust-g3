...
class AuthTest(unittest.TestCase):
    ...
    def test_existing_admin_is_not_revalidated_or_overwritten(self):
        os.environ["ADMIN_PASSWORD"] = "old"
        auth.init_auth(self.app, self.app.config["AUTH_DB_PATH"])
        with self.app.app_context():
            self.assertIsNotNone(auth.authenticate("admin", "strong-admin-password"))
